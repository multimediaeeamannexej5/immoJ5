...
import { CheckCircle2, XCircle, AlertTriangle, ChevronDown, Loader2, FileText, ExternalLink } from 'lucide-react'
...
export default function TreasurerDonationActions({ donationId, currentStatus, adminNotes, notes, proofUrl }: Props) {
    ...
    <div className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-8 z-50 w-72 bg-[#14151E] border border-[#252637] rounded-xl shadow-xl p-3">
          ...
          {proofUrl && (
            <div className="mb-3">
              <p className="text-gray-500 text-xs mb-1.5 font-medium uppercase tracking-wide">Preuve de paiement</p>
              <a href={proofUrl} target="_blank" rel="noopener noreferrer"
                className="flex items-center gap-2 p-2 rounded-lg bg-accent/10 border border-accent/25 text-accent text-xs hover:bg-accent/15 transition-colors">
                <ExternalLink className="w-3.5 h-3.5 flex-shrink-0" />
                Ouvrir la preuve dans un nouvel onglet
              </a>
            </div>
          )}
