...
export default function DonationActions({
  donationId, currentStatus, adminNotes, notes, proofUrl, readOnly,
}: Props) {
      ...
      <div className="flex items-center gap-2">
        {(notes || proofUrl) ? (
          <div className="flex items-center gap-1.5">
            ...
            {proofUrl && (
              <a href={`/api/proof?id=${donationId}`} target="_blank" rel="noopener noreferrer"
                className="text-accent text-xs hover:underline flex items-center gap-0.5">
                <ExternalLink className="w-3 h-3" /> Preuve
              </a>
            )}
          </div>
        ) : (
          ...
          {proofUrl && (
            <div className="mb-3">
              ...
              {showInline ? (
                ...
                /* Fallback : PDF, HEIC sur Chrome, ou image échouée */
